      ...
      <div class="card-body p-2 modal-body flex-fill">
        <h5 class="text-center text-muted-light py-3 px-4 m-0">
          ...
          <span class="text-dark font-weight-bold">{{
            octobayTwitterAccount.followers_count
          }}</span>
          followers.
        </h5>
        <div class="pt-3 pb-4 d-flex">
          <div>
            <div
              ...
              :style="`background-image: url(${octobayTwitterAccount.profile_image_url})`"
            ></div>
          </div>
          <div class="ml-2">
            <div>
            ...
            <a
              :href="`https://github.com/${issue.owner}/${issue.repository}/issues/${issue.number}`"
              target="_blank"
              >https://github.com/{{ issue.owner }}/{{
                issue.repository
              }}/issues/{{ issue.number }}</a
            >
            <a href="https://twitter.com/hashtag/ethereum" target="_blank"
              >#ethereum</a
            >
            <a href="https://twitter.com/hashtag/github" target="_blank"
              >#github</a
            >
            <a href="https://twitter.com/hashtag/opensource" target="_blank"
              >#opensource</a
            >
          </div>
        </div>
        <div>
          ...
          >
            ...
            {{ octobayTwitterAccount.followers_count }} OPIN tokens to post on
            Twitter.
            <a href="https://app.uniswap.org" target="_blank"
              >Buy OPIN on Uniswap.</a
            >
          </small>
          <transition name="fade" mode="out-in">
            <div v-if="tweetId">
              <a
                :href="'https://twitter.com/OctoBayApp/status/' + tweetId"
                target="_blank"
                class="d-flex justify-content-between align-items-center btn btn-twitter btn-lg shadow-sm w-100"
              >
            ...
            <button
              v-else
              class="btn btn-twitter btn-lg shadow-sm w-100"
              :disabled="
                requestingTweet ||
                postingTweet ||
...
export default {
  ...
  mounted() {
    this.$axios
      .$get(process.env.API_URL + '/twitter/user/1333035957805862915')
      .then((account) => {
      ...
      const confirmListener = this.$octoBay.events
        ...
        .on('data', (event) => {
          if (event.returnValues.issueId === this.issue.id) {
            ...
            this.tweetId = this.$web3.utils.hexToAscii(
              event.returnValues.tweetId
            )
