<template>
  <transition name="fade" mode="in-out">
    <div
      v-if="showOracleList"
      class="overlay"
      @click="$store.commit('setShowOracleList', false)"
    >
      <div class="card shadow-sm oracle-list" @click.stop>
        <div class="card-body p-2">
          <h5 class="text-center text-muted-light py-3 px-5">
            Do you want to support a certain oracle?
          </h5>
          <div
            :class="
              'mt-2 text-left d-flex align-items-center btn btn-' +
              (activeOracle == null ? 'primary' : 'outline-light text-dark')
            "
            @click="
              $store.commit('setActiveOracle', null)
              $store.commit('setShowOracleList', false)
            "
          >
            ...
            <div class="ml-2 d-flex flex-column">
              Random
              <small
                :class="'text-' + (activeOracle == null ? 'white' : 'muted')"
                >For every action that involves an oracle, choose a random
                one.</small
              >
            </div>
          </div>
          <div
            ...
            "
            @click="
              $store.commit('setActiveOracle', oracle)
              $store.commit('setShowOracleList', false)
            "
          >
